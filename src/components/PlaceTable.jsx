  ...
  return (
    <Box>
      <Table.Root>
        <Table.Header>
          <Table.Row>
            {tableHeaderInfos.map((header, index) => (
        ...
        <Table.Body>
          {infoMocking.map((place) => {
            return (
              <Table.Row key={place.placeId}>
                <img src={place.mainImage} style={{ width: "500px" }} />
                <Table.RowHeaderCell>{place.name}</Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.rating}</Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.latitude}</Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.longitude}</Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.description}</Table.RowHeaderCell>
                <Table.RowHeaderCell>
                  {place.address.neighborhood}
                </Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.address.city}</Table.RowHeaderCell>
                <Table.RowHeaderCell>{place.address.state}</Table.RowHeaderCell>
                <Table.RowHeaderCell>
